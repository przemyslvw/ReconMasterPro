...
public class SettingsManager {
    ...
    public String getAiGoogleKey() {
        String v = callbacks.loadExtensionSetting(KEY_AI_GOOGLE_KEY);
        return v != null ? v : DEFAULT_AI_GOOGLE_KEY;
    }

    public void setAiGoogleKey(String key) {
        if (key == null) key = "";
        callbacks.saveExtensionSetting(KEY_AI_GOOGLE_KEY, key);
    }

    // ── AI DeepSeek Key ──────────────────────────────────────────────────

    public String getAiDeepSeekKey() {
        String v = callbacks.loadExtensionSetting(KEY_AI_DEEPSEEK_KEY);
        return v != null ? v : DEFAULT_AI_DEEPSEEK_KEY;
    }

    public void setAiDeepSeekKey(String key) {
        if (key == null) key = "";
        callbacks.saveExtensionSetting(KEY_AI_DEEPSEEK_KEY, key);
    }

    // ── AI Local URL ─────────────────────────────────────────────────────

    public String getAiLocalUrl() {
        String v = callbacks.loadExtensionSetting(KEY_AI_LOCAL_URL);
    ...
    public void setAiLocalUrl(String url) {
        ...
        callbacks.saveExtensionSetting(KEY_AI_LOCAL_URL, url.trim());
    }

    // ── AI Provider ──────────────────────────────────────────────────────

    public AiProvider getAiProvider() {
        String v = callbacks.loadExtensionSetting(KEY_AI_PROVIDER);
    ...
    public void setAiProvider(AiProvider provider) {
        ...
        callbacks.saveExtensionSetting(KEY_AI_PROVIDER, provider.name());
    }

    // ── AI Model ─────────────────────────────────────────────────────────

    public String getAiModel() {
        String v = callbacks.loadExtensionSetting(KEY_AI_MODEL);
    ...
    public void setAiModel(String model) {
        ...
        callbacks.saveExtensionSetting(KEY_AI_MODEL, model.trim());
    }

    // ── AI OpSec Mask Secrets ───────────────────────────────────────────

    public boolean isAiMaskSecrets() {
        String v = callbacks.loadExtensionSetting(KEY_AI_MASK_SECRETS);
        return v == null ? DEFAULT_AI_MASK_SECRETS : Boolean.parseBoolean(v);
    }

    public void setAiMaskSecrets(boolean mask) {
        callbacks.saveExtensionSetting(KEY_AI_MASK_SECRETS, String.valueOf(mask));
    }

    // ── AI OpSec Mask Domains ───────────────────────────────────────────

    public boolean isAiMaskDomains() {
        String v = callbacks.loadExtensionSetting(KEY_AI_MASK_DOMAINS);
        return v == null ? DEFAULT_AI_MASK_DOMAINS : Boolean.parseBoolean(v);
    }

    public void setAiMaskDomains(boolean mask) {
        callbacks.saveExtensionSetting(KEY_AI_MASK_DOMAINS, String.valueOf(mask));
    }
    ...
    public void resetToDefaults() {
        ...
        callbacks.saveExtensionSetting(KEY_AI_GOOGLE_KEY,  DEFAULT_AI_GOOGLE_KEY);
        callbacks.saveExtensionSetting(KEY_AI_DEEPSEEK_KEY, DEFAULT_AI_DEEPSEEK_KEY);
        callbacks.saveExtensionSetting(KEY_AI_LOCAL_URL,   DEFAULT_AI_LOCAL_URL);
        callbacks.saveExtensionSetting(KEY_AI_PROVIDER,    DEFAULT_AI_PROVIDER);
        callbacks.saveExtensionSetting(KEY_AI_MODEL,       DEFAULT_AI_MODEL);
        callbacks.saveExtensionSetting(KEY_AI_MASK_SECRETS, String.valueOf(DEFAULT_AI_MASK_SECRETS));
        callbacks.saveExtensionSetting(KEY_AI_MASK_DOMAINS, String.valueOf(DEFAULT_AI_MASK_DOMAINS));
    }
}
